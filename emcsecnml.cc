...
#include "emcsecnml.hh"             // Common NML communications functions
...
int emcTaskNmlGet(int channel)
{
	...
	if(channel==0){
		strcpy(processName,"xemc");
		strcpy(commandName,"emcCommand");
		strcpy(statusName,"emcStatus");
	}
	else{
		sprintf(processName, "xemc%d", channel);
		sprintf(commandName, "emcCommand%d",channel);
		sprintf(statusName, "emcStatus%d", channel);
	}
    // try to connect to EMC cmd
    if (emcCommandBuffer == 0) {
	emcCommandBuffer =
	    new RCS_CMD_CHANNEL(emcFormat, commandName, processName,
				EMC_NMLFILE);
	if (!emcCommandBuffer->valid()) {
	    delete emcCommandBuffer;
	    emcCommandBuffer = 0;
		//printf("create channel %s:%s fail\n",processName, commandName);
	    retval = -1;
	}
    }
	...
	emcStatusBuffer =
	    new RCS_STAT_CHANNEL(emcFormat, statusName, processName,
				 EMC_NMLFILE);
...
int emcErrorNmlGet(int channel)
{
    int retval = 0;
	char processName[256];
	char errorName[256];
	if(channel==0){
		strcpy(processName, "xemc");
		strcpy(errorName, "emcError");
	}
	else{
		sprintf(processName, "xemc%d", channel);
		sprintf(errorName, "emcError%d", channel);
	}
    if (emcErrorBuffer == 0) {
	emcErrorBuffer =
	    new NML(nmlErrorFormat, errorName, processName, EMC_NMLFILE);
	if (!emcErrorBuffer->valid()) {
	    delete emcErrorBuffer;
	    emcErrorBuffer = 0;
		//printf("create channel %s:%s fail\n",processName, errorName);
	    retval = -1;
	}
    }
...
int sendJogCont(int axis, double speed)
{
	...
	switch (axis) {
	case 0:
	    emc_set_teleop_vector.vector.tran.x = speed;
	    break;
	case 1:
	    emc_set_teleop_vector.vector.tran.y = speed;
	    break;
	case 2:
	    emc_set_teleop_vector.vector.tran.z = speed;
	    break;
	case 3:
	    emc_set_teleop_vector.vector.a = speed;
	    break;
	case 4:
	    emc_set_teleop_vector.vector.b = speed;
	    break;
	case 5:
	    emc_set_teleop_vector.vector.c = speed;
	    break;
	}
	emcCommandBuffer->write(emc_set_teleop_vector);
    }
...
int sendJogAbs(int axis, double speed, double pos)
{
    ...
    emc_axis_abs_jog_msg.vel = speed;
